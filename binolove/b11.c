...
int main (int argc, char *argv[])
{
    ...
    int narrwd=VBARNUMS + (1<<(2-1)); /* node seq 1,2 i.e first two levels, which equates to three entries vert, two entries hor*/
    int narrhe=num_vpos + (2<<1)-1; /* node seq 1,2 i.e first two levels, which equates to three entries vert, two entries hor*/
    int *nthpos, *ntvpos;/*  these are tot arrays, but I'm dropping the toti, the n stands for new */ 
    ntvpos=calloc(num_vpos*SQBLX, sizeof(int));
    nthpos=calloc(VBARNUMS*SQBLX, sizeof(int));
    ...
    cairo_surface_write_to_png (surface, "b11.png");
    cairo_destroy (cr);
