/*  this "b" file is actually to try and sort out core dumps I'm getting with spir3. */
...
#define SUBDIV 4
#define NUMCOLS 4 /* 4 repeating colors is abotu right. */
...
int main (int argc, char *argv[])
{
    ...
    int curcentx=blockw/2, curcenty= - blockh/2; /*  yahoo, this works, initial point is above the iamge .. avoids branching later */

    /*  build up the rgb color array */
    colr **col;
    col=malloc(sizeof(colr *) * NUMCOLS * SUBDIV);
    for(i=0;i<NUMCOLS*SUBDIV;i++) {
    ...
    for(j=0;j<SUBDIV;j++) {
        ...
        curcentx = (-1 + 2*(j%2)) * blockw;
        curcenty = (1 - j%2)*blockh;
    ...
    cairo_destroy(cr);
    cairo_surface_write_to_png (surface, "s2.png");
