...
#define MAXREPROS 32  /* number of reproductions of the core shape */
#define NUMCOLS MAXREPROS/2 /* number of colors to use */
...
int main (int argc, char *argv[])
{
    ...
    int ci;
    double radb=60,rads=5, pencil=10, xsta=0,ysta=0, t=0;

    cairo_save(cr);
    for ( i = 0; i < MAXREPROS; i++) {
        ci =i%NUMCOLS;
        t=i*2*M_PI/MAXREPROS;
        xsta=(radb-rads)*cos(t) + pencil*cos((radb-rads)*t/rads);
        ysta=(radb-rads)*sin(t) - pencil*sin((radb-rads)*t/rads);
        cairo_line_to(cr, xsta, ysta);
